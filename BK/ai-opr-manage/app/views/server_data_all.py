...
@require_http_methods(["POST"])
def get_server_data_indicator_monitoring(request):
    """
    获取服务器系统指标监控折现图数据
    """
    try:
        server_data_all_list = ServerDataAll.objects.filter(isDeleted=0).order_by("-updatedTime")
        paginator = Paginator(server_data_all_list, 10)
        server_data_all_list = paginator.page(1)
        data_list = []
        if server_data_all_list:
            for item in server_data_all_list:
                data_list.append({
                    ...
                    "updatedTime": item.updatedTime.strftime("%m-%d %H:%M:%S"),
                })
        data_list.sort(key=functools.cmp_to_key(compare_time_size))
        # 获取网络设备监测指标折线图数据
        return json_resp("success", data_list)
    except Exception as e:
        print(e)
        return json_resp("error", None, 500)


def compare_time_size(x, y):
    tm1 = time.strptime(x.get("time"), "%Y-%m-%d %H:%M:%S")
    tm2 = time.strptime(y.get("time"), "%Y-%m-%d %H:%M:%S")
    return int(time.mktime(tm1))-int(time.mktime(tm2))
