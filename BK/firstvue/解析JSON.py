date = {
  ...
  "permission": "null",
  "result": "true",
  "request_id": "f5f86a76178c457b8414731fad875fc1",
  "message": "success",
  "data": {
    "count": 6,
    "info": [
...
for info in date.get("data").get("info"):
    ...
    for topo in info["topo"]:
        bk_set_id = topo["bk_set_id"]
        bk_set_name = topo["bk_set_name"]
        set_name_list.append(bk_set_name)

        # 模块
        for mod in topo["module"]:
            bk_module_name = mod["bk_module_name"]
            bk_module_id = mod["bk_module_id"]
            model_name_list.append(bk_module_name)
...
set_names = list(set(set_name_list))
...
#                                                   "bk_set_id": bk_set_id,
#                                                   "bk_set_name": bk_set_name,
#                                                   "bk_module_name": bk_host_innerip,
#                                                   "bk_module_id": bk_module_id,
#                                               }......
#
#                                             ]
...
for name in set_names:
    for data in all_data_list:
        if data["bk_set_name"] in total_dict:
            for i in total_dict[data["bk_set_name"]]:
                if data["bk_module_name"] in i:
                    if data not in i[data["bk_module_name"]]:
                        i[data["bk_module_name"]].append(data)
                    break
            else:
                total_dict[data["bk_set_name"]].append({data["bk_module_name"]: [data]})
        else:
            total_dict[data["bk_set_name"]] = []
            total_dict[data["bk_set_name"]].append({data["bk_module_name"]: [data]})

filename = 'a'

with open(f"{filename}.json", "w", encoding="utf8") as fp:
  fp.write(json.dumps(total_dict, indent=4, ensure_ascii=False))
