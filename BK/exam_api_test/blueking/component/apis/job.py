...
class CollectionsJOB(object):
    ...
    def __init__(self, client):
        ...
        self.get_account_list = ComponentAPI(
            ...
            description=u'查询业务下的执行账号 查询业务下用户有权限的执行账号列表'
        )

        self.get_cron_list = ComponentAPI(
            client=self.client, method='GET',
            path='/api/c/compapi/v2/jobv3/get_cron_list/',
            description=u'查询业务下定时作业信息'
        )

        self.get_cron_detail = ComponentAPI(
            client=self.client, method='GET',
            path='/api/c/compapi/v2/jobv3/get_cron_detail/',
            description=u'查询定时作业详情'
        )

        self.save_cron = ComponentAPI(
            client=self.client, method='POST',
            path='/api/c/compapi/v2/jobv3/save_cron/',
            description=u'新建或保存定时作业；新建定时作业，定时任务状态默认为暂停。'
        )

        self.get_job_plan_list = ComponentAPI(
            client=self.client, method='GET',
            path='/api/c/compapi/v2/jobv3/get_job_plan_list/',
            description=u'查询执行方案列表'
        )

        self.get_job_plan_detail = ComponentAPI(
            client=self.client, method='GET',
            path='/api/c/compapi/v2/jobv3/get_job_plan_detail/',
            description=u'根据作业执行方案 ID 查询作业执行方案详情'
        )

        self.update_cron_status = ComponentAPI(
            client=self.client, method='POST',
            path='/api/c/compapi/v2/jobv3/update_cron_status/',
        ...
        self.execute_job_plan = ComponentAPI(
            client=self.client, method='POST',
            path='/api/c/compapi/v2/jobv3/execute_job_plan/',
            description=u'执行作业执行方案 启动作业执行方案'
        )

        self.get_job_instance_status = ComponentAPI(
            client=self.client, method='GET',
            path='/api/c/compapi/v2/jobv3/get_job_instance_status/',
            description=u'根据作业实例 ID 查询作业执行状态'
        )
        ...
        self.get_job_instance_ip_log = ComponentAPI(
            ...
            description=u'根据作业实例 ID 查询作业执行日志 根据ip查询作业执行日志'
        )
