<template>
  ...
  <div class="customCheckContainer">
    <!-- 表单-->
    <div class="customCheckFormContent">
    ...
    <!-- 表格 -->
    <div class="customCheckTableContent">
      <!-- 新增按钮 -->
      <div class="addCustomCheckBtn f12" @click="handleAddCustomCheck">
        + 新增自定义巡检项
      </div>
      <!-- /新增按钮 -->
    ...
    <div>
      <bk-sideslider :is-show.sync="isShow" :title="dialogTitle" :quick-close="true" width="700">
        <div slot="content" class="dialogContent">
          <bk-form :label-width="120" :model="customCheckForm" :rules="rules" ref="validateForm">
            <bk-form-item :error-display-type="'normal'" label="巡检项名称" :required="true" :property="'quota_name'">
              <bk-input v-model="customCheckForm.quota_name" :disabled="!isAdd"></bk-input>
            </bk-form-item>

            <bk-form-item :error-display-type="'normal'" label="巡检对象" :required="true" :property="'quota_os'">
              <bk-select v-model="customCheckForm.quota_os" :disabled="!isAdd">
                <bk-option
            ...
            <bk-form-item :error-display-type="'normal'" label="脚本类型" :required="true" :property="'script_type'">
              <bk-select v-model="customCheckForm.script_type">
                <bk-option
            ...
            <bk-form-item :error-display-type="'normal'" label="脚本内容" :required="true" :property="'script_content'">
              <bk-input :type="'textarea'" :rows="5" v-model="customCheckForm.script_content"></bk-input>
            </bk-form-item>

            <bk-form-item label="是否设置对比">
              <bk-switcher
                v-model="customCheckForm.isSetHandler"
                theme="primary"
                :show-text="true"
                :on-text="'是'"
                :off-text="'否'"
            ...
            <bk-form-item label="对比方式" :property="'quota_handler'">
              <bk-select
                v-model="customCheckForm.quota_handler"
                :disabled="!customCheckForm.isSetHandler"
            ...
            <bk-form-item label="对比值" :property="'quota_threshold'">
              <bk-input
                :placeholder="' '"
                v-model="customCheckForm.quota_threshold"
                :disabled="!customCheckForm.isSetHandler">
              </bk-input>
            </bk-form-item>
...
<script>
    ...
    export default {
        ...
        watch: {
            isShow (newVal) {
                if (!newVal) {
                    this.clearCustomCheckForm()
                }
            },

            // 用户设置对比方式时 对比方式和对比值不能为空 否则可为空
            'customCheckForm.isSetHandler': {
                deep: true,
                handler (val) {
        ...
        methods: {
            ...
            async fetch_custom_quota (param = {}) {
                ...
                try {
                    ...
                    const { data } = res
                    this.customCheckList = data.data
            ...
            handleEdit (param) {
                ...
                keys(param).forEach(k => {
                    if (this.customCheckForm.hasOwnProperty(k)) {
                        this.customCheckForm[k] = param[k]
                        if (param['quota_threshold'] || param['quota_handler'] !== 'cmp_show') {
                            this.customCheckForm.isSetHandler = true
                        }
                        if (k === 'quota_os') this.customCheckForm['quota_os'] = +param['quota_os_id']
                        if (k === 'quota_handler' && param['quota_handler'] === 'cmp_show') this.customCheckForm[k] = ''
                    }
                })
            ...
            setDialogConfig (type = 0) {
                this.isAdd = true
                this.dialogTitle = '新增自定义巡检项'

                if (type !== 0) {
                    this.isAdd = false
                    this.dialogTitle = '编辑自定义巡检项'
                }
            ...
            handleSubmit () {
                this.$refs.validateForm.validate().then(async () => {
                    const { customCheckForm, isAdd, quotaId } = this
                    const copyFormData = JSON.parse(JSON.stringify(customCheckForm))
            ...
            clearCustomCheckForm () {
                const { keys } = Object

                keys(this.customCheckForm).forEach(k => {
                    this.customCheckForm[k] = ''

                    if (k === 'isSetHandler') this.customCheckForm[k] = false
                })
            }
        }
    }
