...
@task()
def task_celery(task_obj, task_id, user):
...
class ExcuteFlow():
    ...
    def detection(self):
        if self.task_obj.exec_schedule == "instant":
            self.execution(self.task_obj)
        else:

            if self.task_obj.exec_schedule == "interval":
                self.timing_execute(self.task_obj, self.task_id)
            elif self.task_obj.exec_schedule == "crontab":
                self.period_execute(self.task_obj, self.task_id, self.user)
    ...
    def execution(self, task):
        ...
        for index, quota in enumerate(self.refrefsh_quota_list()):
            ...
            if index >= len(self.refrefsh_quota_list()) - 1:
                ...
                # 通知
                send_notify(self.task_id)
                return

    # 定时执行
    def timing_execute(self, task, task_id):
    ...
    # 周期执行
    def period_execute(self, task, task_id, user):
        ...
        # 启动时间
        time_time = time.mktime(time.strptime(task.exec_start_time, "%Y-%m-%d %H:%M:%S"))
        ...
        while task_status:
            ...
            if (time.time() - float(time_time)) >= 0:  # 10
                # 获取执行状态
                redis_task = eval(str(redis_link.get(name=task_id), 'utf-8'))

                # 判断状态如果是完成就终止，其他接口改变redis中对应任务id里的数值
                if redis_task['status'] == "executed":
                    redis_link.delete(task_id)
                    # 停止循环
                    task_status = False
                ...
                for index, quota in enumerate(self.refrefsh_quota_list()):
                    ...
                    if index == len(self.refrefsh_quota_list()) - 1:
                        ...
                        # 下一次执行的时间
                        # time_time = time.time() + (int(task.exec_timece) * 86400)  # 11 12

                        # 当前时间 + 周期天数 = 执行日期
                        # time_time = str(time_time + (int(task.exec_timece) * 86400))  # now_time + 1 * 86400
                        time_time = str(time.time() + 80)
                        redis_link.set(name=task_id,
                                       value=f"{{'time_second':{time_time}, 'plan':'crontab', 'status': 'unexecuted'}}")
